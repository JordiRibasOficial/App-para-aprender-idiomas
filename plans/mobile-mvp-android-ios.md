...
- **Alojamiento de política de privacidad/términos [HECHO — falta 1 acción manual del usuario]:** página estática (`index.html`, `privacy.html`, `terms.html`) en una rama `gh-pages` dedicada, deliberadamente separada de `docs/` en `main` para no exponer los documentos de negocio/financieros internos vía Pages. Falta que el usuario active GitHub Pages en Settings → Pages (Source: `gh-pages` / `root`) — instrucciones exactas en `docs/business/store-listing.md`. También pendientes de un dato real del usuario (no inventados): nombre legal/razón social y email de contacto, marcados como "pendiente de confirmar" en las páginas ya publicadas.
- **Pendiente, bloqueado por assets que no existen todavía:** icono final y gráfico de feature (necesitan un logo real, el proyecto sigue con el icono por defecto del scaffold), capturas de pantalla (necesitan un dispositivo/emulador gráfico real, este contenedor no tiene uno).
