...
- **Primer intento real de CI (commit `91a2c77`) falló en ambas plataformas**, en la misma aserción exacta: justo después de simular el reinicio de la app, `find.text('Inglés · A1')` no encontraba nada. Diagnóstico por descubrimiento, no por suposición: un primer arreglo (`76c2bd3`, esperar de forma paciente/con polling en vez de un `pumpAndSettle()` único) resultó **insuficiente** — el siguiente run de CI demostró que el test seguía sin encontrar la pantalla incluso esperando 5s, lo que probaba que no era un problema de timing. La causa raíz real: `appRouter` es un singleton de GoRouter, e `integration_test` ejecuta todo el archivo de test en un único proceso — así que la ubicación del router sobrevivía a la simulación de "reinicio" (se quedaba en la ruta del ejercicio), `RootScreen` nunca se reconstruía, y ninguna espera iba a hacer aparecer la lista de lecciones. Arreglo real (`91453c8`): `appRouter.go('/')` explícito antes del `pumpWidget` de reinicio, simulando de verdad lo que hace un reinicio en frío (proceso nuevo, router nuevo, arranca en `/`).
- **Verificado en verde de verdad**, no solo escrito: tras una incidencia mayor de GitHub Actions (ver Paso 15 para el detalle completo — ~15:22 a ~22:18 UTC del 6 de agosto), el run [31129057557](https://github.com/JordiRibasOficial/App-para-aprender-idiomas/actions/runs/31129057557) terminó con los 4 jobs en verde (`build-android`, `build-ios`, `integration-test-android`, `integration-test-ios`) contra el commit final (`1a36292`, que incluye el fix del router + la funcionalidad de AdMob del Paso 15). Confirmación real, en dispositivo/emulador real, del criterio de salida original del usuario.
...
- **Capturas de pantalla [HECHO — 7/7 reales]:** el contenedor no tiene emulador gráfico, así que la primera pasada usó el target Flutter Web + Playwright (390×844, no el tamaño real de dispositivo). Superado desde entonces: `.github/workflows/store-screenshots.yml` (`workflow_dispatch`) captura las 7 en un emulador Android real en CI (perfil Pixel 6, 1080×2337) vía `flutter drive` — costó 13 ejecuciones encontrando y arreglando bugs reales (`convertFlutterSurfaceToImage()` que faltaba, estado de router/`SharedPreferences` persistiendo entre tests, `takeScreenshot()` devolviendo bytes obsoletos entre testWidgets del mismo proceso y, la última capa, entre procesos `flutter drive` distintos si el compositor de Android no había terminado de pintar — arreglado con un `flutter drive` separado por captura más una pausa real de 1s antes de cada `takeScreenshot()`). Las 7 en `docs/business/store-screenshots/` tienen SHA-256 distintos, verificados uno por uno. Limitación honesta que queda: son capturas de un dispositivo Android (Pixel 6) — Apple exige tamaños de pantalla propios (6.7"/6.5"/5.5") que haría falta capturar por separado desde un Simulador iOS antes de la submission a App Store, algo sin sentido priorizar mientras la cuenta de Apple Developer siga deliberadamente en pausa (ver más abajo).
...
### Criterio de salida
Paquete de submission listo para que el usuario suba a un track interno/TestFlight en cuanto tenga las cuentas. **Parcialmente cumplido** — textos y assets gráficos (icono, feature graphic, 7/7 capturas reales) listos; solo falta la cuenta de Apple Developer (pausa deliberada del usuario, no bloqueo técnico) y la revisión legal real de los borradores.
...
## Paso 15 — Publicidad de terceros en la versión gratuita (AdMob) [HECHO — código verificado en CI real, política de privacidad en vivo ya actualizada; solo falta la cuenta de AdMob]
...
- Verificado localmente: `flutter analyze` limpio, 69/69 tests (2 nuevos, prueban el *gating* por Premium sin tocar nunca el SDK real de anuncios — ver comentarios en `test/premium_gated_banner_ad_test.dart`), `flutter build appbundle --release` compila y enlaza el SDK nativo sin problema (`.aab` de 56.4MB).
- **Verificado en CI real** (run [31129057557](https://github.com/JordiRibasOficial/App-para-aprender-idiomas/actions/runs/31129057557), commit `1a36292`): `build-ios` en verde — primera confirmación de que `google_mobile_ads` enlaza limpio en iOS con `GADApplicationIdentifier` en `Info.plist`, algo que este contenedor no podía comprobar sin Xcode. `build-android`, `integration-test-android` e `integration-test-ios` también en verde en el mismo run — la app arranca y corre sin crashear con el SDK de anuncios enlazado (aunque desactivado por defecto, ver `adsEnabledProvider`) en emulador Android y Simulador iOS reales.
...
**Incidencia de GitHub Actions durante la verificación (6 de agosto, ~15:22–22:18 UTC):** una interrupción a nivel de plataforma (no de este repo) afectó Actions y Pages a la vez. Síntomas reales observados y confirmados contra los propios partes de estado de GitHub, no supuestos:
- El push del commit con AdMob (`479988d`) **nunca disparó ningún check-run** — GitHub confirmó que solo procesaba ~15% de los webhooks. Se añadió `workflow_dispatch` a `mobile-ci.yml` (commit `1a36292`) para poder lanzar el workflow directamente por API, sin depender del webhook — mejora que queda permanente, útil más allá de esta incidencia puntual.
- Un primer run lanzado por API quedó con `integration-test-android` en estado `cancelled` sin runner asignado nunca (`runner_id: 0`) — coincide con el propio parte de incidencia de GitHub ("runners are being assigned invalid jobs").
- Un segundo intento del job de iOS quedó colgado 30+ min sin completar — se canceló y se relanzó, esta vez terminó en ~6 min (rango normal).
- Ningún reintento fue a ciegas: cada reintento se basó en confirmar primero la mejora real reportada por GitHub (tasa de éxito subiendo del 30-40% al 65% y luego al 97%) antes de repetir la acción, no en "probar otra vez a ver si cuela".

**Resuelto desde la nota anterior:**
- ~~La política de privacidad publicada en vivo (`gh-pages` → `privacy.html`) NO se ha actualizado~~ **[HECHO]** — commit `054ad29` en `gh-pages` ("Disclose Google AdMob advertising and UMP consent in the privacy policy") ya publica la misma sección de anuncios/UMP que `docs/business/privacy-policy-draft.md`. Verificado comparando ambos textos línea a línea. Ya no es un bloqueo legal.
...
### Criterio de salida
Código en verde en CI real (Android + iOS build, más el resto de la suite) [HECHO], política de privacidad en vivo (`gh-pages`) actualizada con la sección de publicidad [HECHO], cuenta de AdMob creada y con IDs reales [PENDIENTE — decisión del usuario].
