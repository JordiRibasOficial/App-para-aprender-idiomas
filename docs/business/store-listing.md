...
- ~~Capturas de pantalla~~ **[HECHO — primera pasada vía Flutter Web, ahora también vía dispositivo real]** — 7 capturas reales (no maquetas) en `docs/business/store-screenshots/`: bienvenida, selección de idioma (con el gate de Premium visible en portugués/francés/japonés), selección de nivel, dos ejercicios distintos, lista de lecciones, pantalla Premium. Primera pasada generada compilando la app con el target Flutter Web (`src/mobile/web/`) y capturándola con un Chromium real vía Playwright (`tools/web-screenshots/`) — a 390×844, no un tamaño de dispositivo real. **Ahora que `integration-test-android` corre limpio en CI**, existe una segunda vía que captura desde el emulador Android real (perfil Pixel 6, 1080×2400 — un tamaño de dispositivo de verdad, no un placeholder): lanzar manualmente el workflow `.github/workflows/store-screenshots.yml` (`workflow_dispatch`), que ejecuta `integration_test/screenshot_test.dart` vía `flutter drive` y sube las capturas como artefacto de CI. Pendiente de ejecutar y sustituir las de `store-screenshots/` por las reales del emulador.
