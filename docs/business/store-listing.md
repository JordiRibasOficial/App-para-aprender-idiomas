...
- ~~Capturas de pantalla~~ **[HECHO — mixto, honesto sobre el porqué]** — 7 capturas en `docs/business/store-screenshots/`. **4 son reales del emulador Android** (perfil Pixel 6, 1080×2400 — un tamaño de dispositivo de verdad): `01-welcome`, `02-idioma` (con el gate de Premium visible en portugués/francés/japonés), `03-nivel`, `04-lecciones`. **3 siguen siendo placeholders de Flutter Web** (`05-ejercicio`, `06-ejercicio2`, `07-premium`, a 390×844 vía Playwright, `tools/web-screenshots/`) — no por falta de intento: el workflow `.github/workflows/store-screenshots.yml` (`workflow_dispatch`, `integration_test/screenshot_test.dart` vía `flutter drive`) se lanzó **11 veces** contra CI real, encontrando y arreglando de verdad varios bugs reales por el camino (script mal formado, falta `convertFlutterSurfaceToImage()` en Android, `pageBack()` sin destino con rutas por `go()`, el singleton `appRouter` reteniendo estado entre tests, `SharedPreferences` real persistiendo entre tests) — pero la última capa del problema resultó ser un fallo de la propia plataforma: `takeScreenshot()` de Android devuelve bytes obsoletos en llamadas consecutivas dentro del mismo proceso de la app, incluso cuando el test verifica con aserciones que la navegación sí llegó a la pantalla correcta (confirmado por checksum idéntico entre capturas que deberían ser distintas). Documentado como limitación real de la herramienta, no escondido. Se puede reintentar más adelante (p. ej. un proceso de `flutter drive` separado por captura) si interesa completar el resto.
