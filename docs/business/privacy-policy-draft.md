...
La aplicación tiene un backend propio (Supabase), con un alcance todavía reducido: gestiona cuentas de usuario, verifica compras de suscripción contra Google Play / Apple, y sirve el contenido de los cursos de pago a quien tiene una suscripción activa. La primera lección y el contenido gratuito nunca requieren cuenta ni tocan este backend en absoluto. Los datos que se recogen son:
...
- **Cuenta con email y contraseña**: si el usuario elige "Registrarse con email" — desde el registro inicial, o cuando la app se lo pide al intentar comprar/restaurar Premium, abrir un idioma de pago o usar "Mis datos" (ver más abajo) — creamos una cuenta real (email y contraseña, gestionada por Supabase Auth — nunca vemos ni guardamos la contraseña en texto plano, la gestiona directamente ese proveedor con hash estándar del sector). Crear cuenta **nunca es obligatorio para usar la primera lección ni el contenido gratuito** — solo lo es para lo anterior. `[PENDIENTE: Google/Apple/Facebook como alternativas de registro están planeadas pero no implementadas todavía — se actualizará esta sección cuando lo estén, con el detalle de qué datos comparte cada proveedor.]`
- **Marketing por email (opcional, solo con cuenta)**: al crear una cuenta, el usuario puede marcar, mediante una casilla separada y no premarcada, que quiere recibir ofertas, promociones y novedades por email. Solo si la marca, guardamos su email en una tabla propia dedicada a este fin exclusivamente — nunca lo inferimos ni lo activamos automáticamente a partir de la cuenta. El usuario puede pedir que dejemos de usar su email para esto (ver sección 6); si borra su cuenta desde "Mis datos", este registro se borra también automáticamente (misma mecánica de borrado en cascada que el resto de esta sección). `[PENDIENTE: el número de teléfono para el mismo fin está planeado pero no implementado todavía — se documentará aquí antes de recogerlo.]`
- **Email de confirmación de compra**: al completar una compra, el email de la cuenta se envía una única vez a nuestro backend, junto con esa compra, para poder enviarle el correo de confirmación que exige el art. 98.7 TRLGDCU (ver `terms-of-service-draft.md` sección 4) — nuestro backend lo usa solo para ese envío puntual, a través de Resend (nuestro proveedor de email transaccional), y **no lo guarda** en ninguna tabla ni base de datos propia (distinto del email de la cuenta en sí, que sí vive en `auth.users`, y del de marketing, que solo vive en su propia tabla si el usuario dio ese consentimiento aparte).
- **Datos de verificación de compra**: comprar o restaurar una suscripción, abrir un idioma de pago (portugués, francés o japonés), y usar "Mis datos" requieren tener una cuenta real (ver arriba) — la app la pide antes de intentar cualquiera de estas cosas, nunca después de haber cobrado. Al comprar/restaurar, la app envía el token de compra a nuestro backend, que lo verifica directamente contra la API oficial de Google Play / Apple App Store y guarda el resultado (plataforma, producto, estado, fecha de expiración) asociado a la cuenta — nunca a un nombre suelto ni a datos de pago (esos los gestionan Google/Apple directamente, nunca los vemos). También se registra la marca de tiempo de cada intento de verificación (sin más datos), únicamente para limitar el abuso de este mecanismo.
- **Qué curso de pago se está descargando**: al abrir un idioma de pago, la app pide su contenido a nuestro backend, que comprueba que la cuenta tiene una suscripción activa antes de responder. Esto revela a nuestro backend qué idioma está estudiando un usuario Premium en el momento de la descarga — no se guarda un historial de esto más allá de los logs técnicos habituales del servidor (ver "Retención de logs" más abajo). El progreso dentro de ese curso sigue siendo 100% local.
...
- Cuenta con email y contraseña: ejecución de un contrato (art. 6.1.b RGPD) — el usuario pide expresamente crear una cuenta y esa cuenta es el servicio solicitado.
- Marketing por email: consentimiento explícito del usuario (art. 6.1.a RGPD), recogido mediante una casilla separada y no premarcada — nunca inferido de la creación de la cuenta ni de ningún otro dato.
...
- Datos de verificación de compra: ejecución del contrato de suscripción (necesario para prestar el servicio de pago) — sin la cuenta y esos datos no podemos confirmar que una compra es genuina ni entregar el contenido de pago correspondiente.
...
- **Supabase** (nuestro proveedor de backend, alojado en la UE): actúa como encargado del tratamiento para la cuenta del usuario, los datos de verificación de compra y el consentimiento de marketing, todos descritos en la sección 2. Sujeto a la [política de privacidad de Supabase](https://supabase.com/privacy). Su Acuerdo de Encargado del Tratamiento (DPA) queda incorporado automáticamente al aceptar sus Términos de Servicio — verificado, no requiere firma ni acción separada nuestra (detalle en `docs/business/records-of-processing-activities.md`).
...
Bajo el RGPD, el usuario tiene derecho a acceder, rectificar, suprimir, limitar el tratamiento y portar sus datos. El progreso de aprendizaje vive localmente, así que desinstalar la app lo elimina. Esto **no aplica igual a una cuenta real**: si el usuario creó una (email/contraseña, y en el futuro Google/Apple/Facebook), esa cuenta y su consentimiento de marketing (si lo dio) viven en nuestro backend, no en el dispositivo — desinstalar la app no la borra, precisamente porque el propósito de tener cuenta es poder reinstalar o cambiar de dispositivo sin perderla. Para acceder a los datos guardados en nuestro backend (sección 2), exportarlos o **eliminarlos permanentemente** (cuenta, suscripción, intentos de verificación de compra y consentimiento de marketing — todo junto, en una sola operación gracias a cómo está diseñada la base de datos), puede usar la opción "Mis datos" dentro de la propia App — el botón "Eliminar mis datos" borra de inmediato, sin necesidad de escribir a nadie; es irreversible. También puede solicitarlo por email a world.webapps@gmail.com indicando la plataforma (Android/iOS) y, si se conoce, la fecha aproximada de la suscripción.
