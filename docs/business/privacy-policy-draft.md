...
La aplicación tiene un backend propio (Supabase), con un alcance todavía reducido: verifica compras de suscripción contra Google Play / Apple, sirve el contenido de los cursos de pago a quien tiene una suscripción activa, y — desde esta versión — puede gestionar una cuenta real si el usuario decide crear una. Los datos que se recogen son:
...
- **Cuenta con email y contraseña (opcional)**: si el usuario elige "Registrarse con email" en el registro inicial, creamos una cuenta real (email y contraseña, gestionada por Supabase Auth — nunca vemos ni guardamos la contraseña en texto plano, la gestiona directamente ese proveedor con hash estándar del sector). El usuario puede seguir usando la app sin crear cuenta ("Continuar como invitado") — crear cuenta nunca es obligatorio para usar la primera lección ni el contenido gratuito. `[PENDIENTE: Google/Apple/Facebook como alternativas de registro están planeadas pero no implementadas todavía — se actualizará esta sección cuando lo estén, con el detalle de qué datos comparte cada proveedor. También pendiente: hoy solo quien crea una cuenta real puede recibir el correo de confirmación de compra descrito más abajo — evaluar si un invitado debería poder darlo también sin crear cuenta.]`
...
- **Email de confirmación de compra (opcional)**: si el usuario tiene una cuenta real y completa una compra, su email de cuenta se envía una única vez a nuestro backend, junto con esa compra, para poder enviarle el correo de confirmación que exige el art. 98.7 TRLGDCU (ver `terms-of-service-draft.md` sección 4) — nuestro backend lo usa solo para ese envío puntual, a través de Resend (nuestro proveedor de email transaccional), y **no lo guarda** en ninguna tabla ni base de datos propia (distinto del email de la cuenta en sí, que si vive en `auth.users`, y del de marketing, que solo vive en su propia tabla si el usuario dio ese consentimiento aparte).
- **Identidad anónima de sesión (solo invitados)**: si el usuario no crea una cuenta, para verificar una compra o descargar un curso de pago la app crea automáticamente una identidad anónima en nuestro backend (Supabase) — un identificador técnico sin nombre, email ni ningún dato personal asociado, que solo sirve para vincular esas dos operaciones a un mismo dispositivo. Esta identidad se guarda cifrada en el dispositivo (Keychain/almacenamiento cifrado).
...
- **Supabase** (nuestro proveedor de backend, alojado en la UE): actúa como encargado del tratamiento para la cuenta del usuario (si crea una), la identidad anónima de sesión (si no), los datos de verificación de compra y el consentimiento de marketing, todos descritos en la sección 2. Sujeto a la [política de privacidad de Supabase](https://supabase.com/privacy). Su Acuerdo de Encargado del Tratamiento (DPA) queda incorporado automáticamente al aceptar sus Términos de Servicio — verificado, no requiere firma ni acción separada nuestra (detalle en `docs/business/records-of-processing-activities.md`).
