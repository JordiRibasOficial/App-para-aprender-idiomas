...
|---|---|---|
| Dirección de email | **Sí, opcional** | Solo si el usuario elige "continuar con email" en el onboarding (`auth_choice_screen.dart`) en vez de "continuar como invitado". Se guarda **únicamente en el dispositivo**, cifrado (`flutter_secure_storage`); no se usa para crear cuenta. **Si el usuario lo dio y completa una compra**, se envía una única vez a nuestro backend junto con esa compra, para el correo de confirmación que exige el art. 98.7 TRLGDCU (vía Resend) — no se guarda en ninguna tabla propia. Marca esta fila como compartida con un tercero (Resend) para esa finalidad puntual en el cuestionario. |
...
| Identificador de sesión anónimo | **Sí** | La app ya tiene backend propio (Supabase), con alcance reducido a verificar compras y servir el contenido de pago — no gestiona cuentas de usuario. Al comprar/restaurar una suscripción o descargar un curso de pago, se crea automáticamente una identidad anónima (sin nombre, email ni dato personal) en ese backend, para vincular esas operaciones a un mismo dispositivo. |
| Historial de compras / facturación | **Sí** | Google Play Billing (`in_app_purchase`) procesa el pago en sí, según su propia política. Además, nuestro backend verifica cada compra directamente contra la API de Google Play y guarda el resultado (plataforma, producto, estado, fecha de expiración) asociado al identificador anónimo de arriba — nunca a un nombre o email. |
...
| Ubicación, contactos, fotos, analítica de terceros | No | No hay ningún SDK de analítica de comportamiento/marketing integrado — solo el SDK de anuncios de arriba y Supabase para lo descrito en las filas anteriores. |
