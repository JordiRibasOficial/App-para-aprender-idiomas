...
|---|---|---|
| Dirección de email | **Sí, opcional** | Solo si el usuario elige "Registrarse con email" en el onboarding (`auth_choice_screen.dart`) en vez de "Continuar como invitado" — crea una cuenta real (Supabase Auth) con ese email, no solo un dato de contacto local. **Finalidad "Funcionalidad de la app"** (la cuenta en sí) y, si el usuario marca la casilla separada de marketing, también **finalidad "Marketing"** (guardado en una tabla propia dedicada, `marketing_contacts`) — marca ambas finalidades en el cuestionario, la de marketing solo aplica a quien la consintió explícitamente. **Si tiene cuenta y completa una compra**, el email se envía además una única vez a nuestro backend junto con esa compra, para el correo de confirmación que exige el art. 98.7 TRLGDCU (vía Resend) — esa transmisión puntual no se guarda en ninguna tabla propia. Marca esta fila como compartida con un tercero (Resend) para esa finalidad puntual en el cuestionario. |
...
| Identificador de sesión anónimo (solo invitados) | **Sí** | Si el usuario no crea una cuenta, al comprar/restaurar una suscripción o descargar un curso de pago se crea automáticamente una identidad anónima (sin nombre, email ni dato personal) en nuestro backend (Supabase), para vincular esas operaciones a un mismo dispositivo. |
