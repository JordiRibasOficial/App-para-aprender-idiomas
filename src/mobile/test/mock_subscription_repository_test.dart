...
void main() {
  group('MockSubscriptionRepository', () {
    ...
    test(
      'entitlementStream stays silent until a purchase or restore happens',
      () async {
        final repository = MockSubscriptionRepository();
    ...
    test(
      'restorePurchases re-emits the current entitlement without changing it',
      () async {
        final repository = MockSubscriptionRepository();
        final emitted = <Entitlement>[];
        final subscription = repository.entitlementStream.listen(emitted.add);
    ...
    test(
      'simulateFailure reports a purchase error instead of granting an entitlement',
      () async {
